...
import reactive.inventory.{MongoRepo, InventoryManager}
import MongoRepo._
 ...
 /* override def afterAll () {
  ...
  "InventoryManager" should {
    "set and get inventory for Sku when asked" in {
      ...
      expectMsg(InventoryResponse(1, "get inventory", "1", true, 5, ""))
    }
    ...
    "remove inventory for Sku when purchased" in {
      ...
      expectMsg(InventoryResponse(1, "buy inventory", "1", true, 3, ""))
    }
    ...
    "return appropriate failure message for Sku when too many purchased" in {
      ...
      expectMsg(InventoryResponse(1, "buy inventory", "1", false, 6, "Only 5 left"))
    }
    ...
    "add inventory for Sku" in {
      ...
      expectMsg(InventoryResponse(1, "add inventory", "1", true, 2, ""))
    }
    ...
    "removing inventory for Sku should decrement inventory" in {
      ...
      expectMsg(InventoryResponse(1, "buy inventory", "1", true, 3, ""))
      inventoryManager ! GetInventory(1)
      expectMsg(InventoryResponse(1, "get inventory", "1", true, 2, ""))
    }
  }
}